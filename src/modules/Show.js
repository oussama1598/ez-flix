...
export default class Show {
  async loadData(showName) {
    ...
    if (!Object.keys(this.showsData).length)
  ...
  getRangeEpisodes(season, fromEpisode = 1, toEpisode = Infinity) {
    ...
    return _.chain(episodes)
      .sortBy('episodes')
      .filter(ep => ep.episode >= fromEpisode && ep.episode <= toEpisode)
      .value();
  }
  ...
  getLastSeasonNumber() {
    return Object.keys(this.showsData).reduce(
      (a, b) => (parseInt(a, 10) > parseInt(b, 10) ? a : b),
      0
    );
  }

  getLastEpisodeNumber(seasonNumber) {
    return Object.keys(this.showsData[seasonNumber]).reduce(
      (a, b) => (a.episode > b.episode ? a.episode : b.episode),
      0
    );
  }
}
